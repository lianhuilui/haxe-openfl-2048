...
Main.prototype = $extend(openfl.display.Sprite.prototype,{
	initialize: function() {
		this.showscoreboard();
		this.showmenu();
		this.stage.addEventListener(openfl.events.KeyboardEvent.KEY_DOWN,$bind(this,this.game_onKeyDown));
	}
	,showscoreoverlay: function() {
		this.scoreOverlay = new openfl.text.TextField();
		var format = new openfl.text.TextFormat();
		format.color = 16711680;
		format.size = 35;
		format.align = openfl.text.TextFormatAlign.CENTER;
		this.scoreOverlay.set_y(250);
		this.scoreOverlay.set_width(Main.NUM_TILES * 100);
		this.scoreOverlay.set_defaultTextFormat(format);
		this.scoreOverlay.set_text("YOU SCORED " + this.game.getScore());
		this.addChild(this.scoreOverlay);
	}
	,showscoreboard: function() {
		this.scoreboard = new openfl.display.Sprite();
		this.scoreText = new openfl.text.TextField();
		this.scoreTextFormat = new openfl.text.TextFormat();
		this.scoreTextFormat.color = 255;
		this.scoreTextFormat.size = 20;
		this.scoreText.set_defaultTextFormat(this.scoreTextFormat);
		this.scoreboard.addChild(this.scoreText);
		this.addChild(this.scoreboard);
	}
	,showoverlay: function() {
		this.overlay = new openfl.display.Sprite();
		this.overlay.get_graphics().beginFill(0,0.75);
		this.overlay.get_graphics().drawRect(0,0,Main.NUM_TILES * 100,Main.NUM_TILES * 100 + Main.scoreboardheight);
		this.addChild(this.overlay);
	}
	,showgameover: function() {
		this.gameoverText = new openfl.text.TextField();
		if(this.textFormat == null) {
			this.textFormat = new openfl.text.TextFormat();
			this.textFormat.color = 16711680;
			this.textFormat.size = 30;
			this.textFormat.align = openfl.text.TextFormatAlign.CENTER;
		}
		this.gameoverText.set_defaultTextFormat(this.textFormat);
		this.gameoverText.set_text("GAME OVER");
		this.gameoverText.selectable = false;
		this.gameoverText.set_y(50 + Main.scoreboardheight);
		this.gameoverText.set_width(100 * Main.NUM_TILES);
		this.gameoverText.set_x(50 * Main.NUM_TILES - this.gameoverText.get_width() / 2);
		this.addChild(this.gameoverText);
	}
	,newgame: function() {
		this.removeChild(this.gamemenu);
		if(this.game == null) this.startgame(); else this.restartgame();
	}
	,restartgame: function() {
		if(this.game != null) this.removeChild(this.game);
		if(this.gameoverText != null) this.removeChild(this.gameoverText);
		if(this.gamemenu != null) this.removeChild(this.gamemenu);
		if(this.scoreOverlay != null) this.removeChild(this.scoreOverlay);
		if(this.overlay != null) this.removeChild(this.overlay);
		this.startgame();
	}
	,startgame: function() {
		this.game = new Game(Main.NUM_TILES,Main.NUM_TILES);
		this.game.set_y(Main.scoreboardheight);
		this.game.onDead($bind(this,this.onDead));
	...
	,showmenu: function() {
		this.gamemenu = new openfl.display.Sprite();
		var button = new Button();
		button.setText("NEW GAME");
		button.onClick($bind(this,this.newgame));
		button.set_y(100 + Main.scoreboardheight);
		button.set_x(50 * Main.NUM_TILES - 100);
		this.gamemenu.addChild(button);
		this.addChild(this.gamemenu);
	}
	,game_onKeyDown: function(event) {
	...
	,updateScore: function() {
		this.scoreText.set_text("Score: " + this.game.getScore());
	}
	,__class__: Main
});
...
var Button = function() {
	...
	this.get_graphics().drawRect(0,0,200,40);
	this.addChild(this.textField);
	this.addEventListener(openfl.events.MouseEvent.CLICK,$bind(this,this.runOnClick));
};
...
Button.prototype = $extend(openfl.display.Sprite.prototype,{
	initialize: function() {
		...
		this.textField.set_x(0);
		this.textField.set_y(7);
		this.textField.selectable = false;
		this.textField.set_defaultTextFormat(format);
		this.textField.set_width(200);
		this.textField.set_height(40);
		this.addChild(this.textField);
	}
	,setText: function(text) {
		this.buttonText = text;
		this.textField.set_text(this.buttonText);
	}
	,getText: function() {
		return this.buttonText;
	}
	,onClick: function(callback) {
		if(this.callbacks == null) this.callbacks = new Array();
		this.callbacks.push(callback);
	}
	,runOnClick: function(event) {
		var _g1 = 0;
		var _g = this.callbacks.length;
		while(_g1 < _g) {
			var i = _g1++;
			this.callbacks[0]();
		}
...
var Game = function(cols,rows) {
	...
	var _g1 = 0;
	var _g = this.NUM_ROWS;
	while(_g1 < _g) {
		var i = _g1++;
		this.tiles.push(new Array());
		var _g3 = 0;
		var _g2 = this.NUM_COLUMNS;
		while(_g3 < _g2) {
			var j = _g3++;
	...
	bg.get_graphics().beginFill(16751018,0.4);
	bg.get_graphics().drawRect(0,0,100 * this.NUM_COLUMNS,100 * this.NUM_ROWS);
	this.addChild(bg);
	this.newtile();
};
$hxClasses["Game"] = Game;
...
Game.prototype = $extend(openfl.display.Sprite.prototype,{
	gravity: function(dir,trial) {
		if(trial == null) trial = false;
		var moved = false;
		switch(dir) {
		case "up":
			var _g1 = 0;
			var _g = this.NUM_COLUMNS;
			while(_g1 < _g) {
				var i = _g1++;
				moved = this.gravity_pull(i,0,0,-1,trial) || moved;
			}
			break;
		case "down":
			var _g11 = 0;
			...
			while(_g11 < _g2) {
				var i1 = _g11++;
				moved = this.gravity_pull(i1,this.NUM_ROWS - 1,0,1,trial) || moved;
			}
			break;
		case "left":
			var _g12 = 0;
			var _g3 = this.NUM_ROWS;
			while(_g12 < _g3) {
				var i2 = _g12++;
				moved = this.gravity_pull(0,i2,-1,0,trial) || moved;
			}
			break;
		case "right":
			var _g13 = 0;
			var _g4 = this.NUM_ROWS;
			while(_g13 < _g4) {
				var i3 = _g13++;
				moved = this.gravity_pull(this.NUM_COLUMNS - 1,i3,1,0,trial) || moved;
			}
			break;
		}
		return moved;
	...
	,move: function(dir) {
		var moved = this.gravity(dir);
		if(this.isDead()) return false;
		if(moved) {
			var random_num_tiles = Std.random(Game.MAX_NUM_NEW_TILES) + 1;
			var _g = 0;
			while(_g < random_num_tiles) {
				var i = _g++;
				this.newtile();
			}
		}
	...
	,next_tile: function(x,y,dir_x,dir_y) {
		if(x < 0 || x >= this.NUM_COLUMNS || y < 0 || y >= this.NUM_ROWS) return null;
		if(this.tiles[x][y] != null) return this.tiles[x][y]; else return this.next_tile(x - dir_x,y - dir_y,dir_x,dir_y);
	}
	,gravity_pull: function(x,y,dir_x,dir_y,trial) {
		if(trial == null) trial = false;
		var affected_tiles = new Array();
		var next_x = x - dir_x;
		var next_y = y - dir_y;
		if(next_x < 0 || next_x >= this.NUM_COLUMNS || next_y < 0 || next_y >= this.NUM_ROWS) return false;
		var current_tile = this.next_tile(x,y,dir_x,dir_y);
		while(current_tile != null) {
			affected_tiles.push(current_tile);
			current_tile = this.next_tile(current_tile.X - dir_x,current_tile.Y - dir_y,dir_x,dir_y);
		}
		...
		while(affected_tiles.length > 0) {
			...
			if(prev != null && prev.tileValue == tile.tileValue) {
				if(!trial) {
					this.addScore(prev.tileValue,tile.tileValue);
					prev.Increment(tile.tileValue);
					if(prev.tileValue > this.maxtile) this.maxtile = prev.tileValue;
					this.tiles[tile.X][tile.Y] = null;
					motion.Actuate.tween(tile,0.3,{ x : prev.get_x(), y : prev.get_y(), alpha : 0}).onComplete((function(tile1) {
						return function() {
							tile1.destroy();
						};
					})(tile));
				...
				if(tile.X != x || tile.Y != y) {
					if(!trial) {
						this.tiles[tile.X][tile.Y] = null;
						motion.Actuate.tween(tile,0.3,{ x : x * 100, y : y * 100});
						this.tiles[x][y] = tile;
						tile.X = x;
						tile.Y = y;
					}
			...
			if(tile != null) prev = tile; else prev = null;
		}
		return moved;
	}
	,isDead: function() {
		if(this.getEmptyTiles() == 0 && !this.canMove("up") && !this.canMove("down") && !this.canMove("left") && !this.canMove("right")) {
			if(this.onDeadCallback != null && !this.callbackCalled) {
				this.onDeadCallback();
	...
	,canMove: function(dir) {
		return this.gravity(dir,true);
	}
	,isFree: function(x,y) {
		if(x >= 0 && x < this.NUM_COLUMNS && y >= 0 && y < this.NUM_ROWS) return this.tiles[x][y] == null;
		return false;
	}
	,tileSame: function(x,y,rhsTile) {
		if(x >= 0 && x < this.NUM_COLUMNS && y >= 0 && y < this.NUM_ROWS) return this.tiles[x][y].tileValue == rhsTile.tileValue;
		return false;
	}
	,getEmptyTiles: function() {
		...
		while(_g1 < _g) {
			...
			while(_g3 < _g2) {
				var j = _g3++;
				if(this.tiles[i][j] == null) emptyTiles++;
			}
		}
		return emptyTiles;
	}
	,tracedebug: function() {
		...
		while(_g1 < _g) {
			...
			while(_g3 < _g2) {
				...
				if(tile != null) line += tile.tileValue; else line += "-";
			}
			haxe.Log.trace(line,{ fileName : "Game.hx", lineNumber : 250, className : "Game", methodName : "tracedebug"});
		}
	}
	,newtile: function() {
		var tile = new Tile();
		var rand_x = Std.random(this.NUM_COLUMNS);
		var rand_y = Std.random(this.NUM_ROWS);
		var upper_limit = 10000;
		while(!this.isFree(rand_x,rand_y) && upper_limit-- > 0) {
			rand_x = Std.random(this.NUM_COLUMNS);
			rand_y = Std.random(this.NUM_ROWS);
		}
		if(this.isFree(rand_x,rand_y)) {
			tile.setPos(rand_x,rand_y);
			this.tiles[rand_x][rand_y] = tile;
			tile.set_scaleX(0);
			tile.set_scaleY(0);
			var _g = tile;
			_g.set_x(_g.get_x() + 50);
			var _g1 = tile;
			_g1.set_y(_g1.get_y() + 50);
			this.addChild(tile);
			motion.Actuate.tween(tile,0.3,{ scaleX : 1, scaleY : 1});
			motion.Actuate.tween(tile,0.3,{ x : tile.get_x() - 50, y : tile.get_y() - 50});
			return true;
		} else return false;
	}
	,addScore: function(first,second) {
	...
	,randomRow: function() {
		return Std.random(this.NUM_ROWS);
	}
	,randomColumn: function() {
		return Std.random(this.NUM_COLUMNS);
	}
	,onDead: function(callback) {
...
var Tile = function() {
	this.colors = [16765301,16750848,10079232,6750207,6737151,10066431,13395609,16737945,52326,3381606,16777215];
...
$hxClasses["Tile"] = Tile;
Tile.__name__ = ["Tile"];
Tile.__super__ = openfl.display.Sprite;
Tile.prototype = $extend(openfl.display.Sprite.prototype,{
	initialize: function() {
		var rand = this.current_color_index = Std.random(this.values.length);
		this.tileValue = this.values[rand];
		this.textformat = new openfl.text.TextFormat();
		this.textformat.size = 40;
		this.textformat.align = openfl.text.TextFormatAlign.CENTER;
		this.text = new openfl.text.TextField();
		this.text.selectable = false;
		this.text.set_defaultTextFormat(this.textformat);
		this.text.set_text("" + this.tileValue);
		this.text.set_y(22);
	...
	,drawBox: function() {
		this.box.get_graphics().beginFill(this.getColor(),1);
		this.box.get_graphics().drawRect(Tile.margin,Tile.margin,Tile.tilesize - Tile.margin * 2,Tile.tilesize - Tile.margin * 2);
	}
	,destroy: function() {
	...
	,setPos: function(x,y) {
		this.X = x;
		this.Y = y;
		this.set_x(x * Tile.tilesize);
		this.set_y(y * Tile.tilesize);
	}
	,getColor: function() {
		return this.colors[this.current_color_index < this.colors.length?this.current_color_index:this.colors.length - 1];
	}
	,Increment: function(rhs) {
		if(rhs == null) rhs = -1;
...
openfl.display.DisplayObject.__worldRenderDirty = 0;
openfl.display.DisplayObject.__worldTransformDirty = 0;
Main.scoreboardheight = 50;
Main.NUM_TILES = 4;
Game.MAX_NUM_NEW_TILES = 1;
Tile.tilesize = 100;
Tile.margin = 4;
